        ...
        function _getEatCuisines(){
          const whitelist=['Italian','Indian','Japanese','Chinese','Spanish','French','Thai','Mediterranean','British','American','Sri Lankan','Middle Eastern','Street Food','Wine Bar','Halal'];
          const map={
        ...
        function _renderEatTab(){
          ...
          const filtered=_eatFilter==='All'?venues:_eatFilter==='Halal'?venues.filter(v=>v.halal===true):venues.filter(v=>{
            const raw=(v.loc||'').split('·')[1]||'';
            const lower=raw.toLowerCase().trim();
        ...
        async function startBetaSignup(intent){
          ...
          // Unified flow: try sign-in first (shouldCreateUser=false), fall back to sign-up
          if(!_sb){btn.textContent='Continue';btn.disabled=false;toast('Not connected');return;}
          const{error:signInErr}=await _sb.auth.signInWithOtp({email,options:{shouldCreateUser:false,emailRedirectTo:window.location.origin+'/?app'}});
          if(!signInErr){
            // Existing user — magic link sent for sign-in
        ...
        // Profile completion — shown post-magic-link for new users without name/handle
